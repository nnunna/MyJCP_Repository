...
CONFIG=/root/naveen/DP/COLA/conf/PROD.txt
DISK_SPACE_REPORT=$TEMP/COLA_consolidated_disk_alert_report.html
MEMORY_USAGE_REPORT=$TEMP/COLA_consolidated_memory_usage_report.html
SWAP_USAGE_REPORT=$TEMP/COLA_consolidated_swap_usage_report.html
...
SUBJECT="$3"
	 ...
	 if [ -f /tmp/COLA_uniq_server_count.txt ];then
	 	 ...
	 	 while read count uniq_mac; do
			 ...
			 echo "</tr>"
		 done < /tmp/COLA_uniq_server_count.txt
		 rm -f /tmp/COLA_uniq_server_count.txt
        	 echo "</table></body></html>"
	 else
	         echo "<html>"
        	 cat $REPORT_FILE | sed -e 's/^/<br>/g'
         	 echo "</html>"
		 touch /tmp/COLA_mem_inc1
	 fi
         echo
        ) | /usr/sbin/sendmail -t
...
do
  ssh -q $server -l root -n "df -PH | grep -w 'usr\|tmp\|var\|home\|opt' | awk '{print \$5, \$6}'" >> $TEMP/COLA_server_disk.txt
...
#        echo -e "<font color="green">$server <font color="blue">$filesystem <font color="red">$use_percent%</font></font></font>" >>$DISK_SPACE_REPORT
  ...
  done < $TEMP/COLA_server_disk.txt
  rm -f $TEMP/COLA_server_disk.txt
done
...
{
   ALERT="Disk Space"
   cat $DISK_SPACE_REPORT | awk '{print $1}' | uniq -c >> /tmp/COLA_uniq_server_count.txt
...
for server in `cat $CONFIG`;
do
	ssh -q $server -l root -n "cat /proc/meminfo | grep 'MemTotal\|MemFree\|SwapTotal\|SwapFree'" >> $TEMP/COLA_server_memory.txt
	i=0
 
	while read mem val junk;do
		mem_val[i]=$val
		i=`expr $i + 1`
	done < $TEMP/COLA_server_memory.txt
...
### If memory usage % is more than the threshold, then report
	...
	rm $TEMP/COLA_server_memory.txt
done
...
##### Cleanup #####
rm -f $TEMP/COLA_server_memory.txt
