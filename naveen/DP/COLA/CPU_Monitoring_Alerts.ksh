...
PROCESS_CPU_UTLIZATION=$TEMP/COLA_consolidated_user_cpu_report.html
SYSTEM_CPU_UTLIZATION=$TEMP/COLA_consolidated_system_cpu_report.html
IO_CPU_UTLIZATION=$TEMP/COLA_consolidated_io_cpu_report.html
...
do
  ssh -q $server -l root -n "sar -u 5 4 | grep -i all | grep -v Average | awk '{print \$1\$2, \$4, \$6, \$7}'" >> $TEMP/COLA_server_cpu.txt
  i=0
