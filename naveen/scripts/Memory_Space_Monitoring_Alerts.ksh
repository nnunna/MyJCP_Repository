...
SUBJECT="$3"
	 ...
	 if [ -f /tmp/uniq_server_count.txt ];then
	 	 ...
	 	 while read count uniq_mac; do
			 ...
			 echo "</tr>"
		 done < /tmp/uniq_server_count.txt
		 rm -f /tmp/uniq_server_count.txt
...
do
  ssh -q $server -l root -n "df -H | grep -w 'usr\|tmp\|var\|home\|opt' | awk '{print \$5, \$6}'" >> $TEMP/server_disk.txt
...
{
   ALERT="Disk Space"
   cat $DISK_SPACE_REPORT | awk '{print $1}' | uniq -c >> /tmp/uniq_server_count.txt
