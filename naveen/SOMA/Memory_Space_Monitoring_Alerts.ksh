...
CONFIG=/root/naveen/SOMA/conf/servers_list.txt
DISK_SPACE_REPORT=$TEMP/soma_consolidated_disk_alert_report.html
MEMORY_USAGE_REPORT=$TEMP/soma_consolidated_memory_usage_report.html
SWAP_USAGE_REPORT=$TEMP/soma_consolidated_swap_usage_report.html
...
#         echo "To: Team <$MAILTO>"
	 ...
	 if [ -f /tmp/soma_uniq_server_count.txt ];then
	 	 ...
	 	 while read count uniq_mac; do
			 ...
			 echo "</tr>"
		 done < /tmp/soma_uniq_server_count.txt
		 rm -f /tmp/soma_uniq_server_count.txt
...
do
  ssh -q $server -l root -n "df -Ph | grep -w 'usr\|tmp\|var\|home\|opt' | awk '{print \$5, \$6}'" >> $TEMP/soma_server_disk.txt
...
#        echo -e "<font color="green">$server <font color="blue">$filesystem <font color="red">$use_percent%</font></font></font>" >>$DISK_SPACE_REPORT
  ...
  done < $TEMP/soma_server_disk.txt
  rm -f $TEMP/soma_server_disk.txt
done
...
{
   ALERT="Disk Space"
   cat $DISK_SPACE_REPORT | awk '{print $1}' | uniq -c >> /tmp/soma_uniq_server_count.txt
...
for server in `cat $CONFIG`;
do
	ssh -q $server -l root -n "cat /proc/meminfo | grep 'MemTotal\|MemFree\|SwapTotal\|SwapFree'" >> $TEMP/soma_server_memory.txt
	i=0
 
	while read mem val junk;do
		mem_val[i]=$val
		i=`expr $i + 1`
	done < $TEMP/soma_server_memory.txt
...
### If memory usage % is more than the threshold, then report
	...
	rm $TEMP/soma_server_memory.txt
done
...
##### Cleanup #####
rm -f $TEMP/soma_server_memory.txt
